...
void renderer_render()
{
    ...
    for (int i = 0; i < entity_array_size; i++)
    {
        ...
        entity_dst_rect.x = entity_array[i]->position.x + entity_array[i]->sprite->offset.x - entity_array[i]->sprite->size.x / 2;
        entity_dst_rect.y = entity_array[i]->position.y + entity_array[i]->sprite->offset.y - entity_array[i]->sprite->size.y / 2;
...
/*     SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
    SDL_RenderDrawLineF(renderer, maya->offset.x, maya->offset.y, maya->offset.x + velocity.x, maya->offset.y + velocity.y);
    SDL_SetRenderDrawColor(renderer, 255, 128, 128, 255);
    SDL_RenderDrawLineF(renderer, 0, 0, maya->offset.x, maya->offset.y); */
